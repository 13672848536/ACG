    ...
    <div id="login" style="note">
        <h1>登录</h1>
          <div>
            ...
            <router-link to="/forget2">忘记密码</router-link>
            <router-link to="/Registered">马上注册</router-link>
...
export default {
  ...
  methods: {
    login: function () {
      ...
      this.axios.post('/api/demo/user/Login', this.UserDTO).then((res) => {
        if (res.data !== null && res.data) {
          var list = res.data
          this.$message({message: '登录成功', type: 'success', duration: 1000})
          this.$router.push({name: 'System', query: {list}})
        } else {
