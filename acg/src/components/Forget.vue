...
<div class="background">
    <img :src="imgSrc" width="100%" height="100%" alt="" />
        ...
        <div class="form">
          <label>姓名：</label><input type="email" v-model="name"><br/>
        </div>
        <div class="form">
          <label>身份证号码：</label><input type="email" v-model="identification"><br/>
        </div>
...
export default {
  data () {
    return {
      ...
      imgSrc: require('../assets/login3.jpeg')
    }
  },
  methods: {
    setUserinfo () {
    ...
    forget () {
      ...
      if (this.password === this.password2) {
        if (sfz.test(this.UserinfoDTO.identification)) {
          this.axios.post('/api/demo/user/forget', qs.stringify(this.UserinfoDTO)).then((rse) => {
            if (rse.data === true) {
              this.$router.push({path: '/AdminLogin'})
            } else {
